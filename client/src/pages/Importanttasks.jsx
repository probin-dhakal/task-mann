...
function Importanttasks() {
  ...
  useEffect(() => {
    const fetchUserTasks = async () => {
      try {
        ...
        // Filter tasks with important === true
        setTasks(data.tasks.filter((task) => task.important));
      } catch (error) {
  ...
  const toggleTaskImportant = async (id, currentImportant) => {
    try {
      ...
      // Update the tasks state and filter out tasks that are no longer important
      setTasks((prevTasks) =>
        prevTasks
          .map((task) =>
    ...
    <div className="grid grid-cols-4 gap-4 p-4">
      {tasks.length > 0 ? (
        ...
        <div className="col-span-4 text-center text-gray-300 bg-gray-800 p-4 rounded-sm">
          <h2 className="text-2xl">NO IMPORTANT MARKED YET</h2>
        </div>
      )}
    </div>
  );
